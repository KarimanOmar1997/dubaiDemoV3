export const llmActions = ({
  ...
  const findNearbyResources = async (lat, lon, resourceType = "all", radius = 5) => {
...
تم عرض المسار على الخريطة مع علامات توضيحية.`
  ...
  const routeToDestination = async (endLat, endLon, startLat = 25.267078, startLon = 55.293646) => {
    try {
      // Validate coordinates
      if (Number.isNaN(startLat) || Number.isNaN(startLon) || Number.isNaN(endLat) || Number.isNaN(endLon)) {
        const retMessage = "⚠️ إحداثيات غير صحيحة. يرجى التحقق من القيم المقدمة.";
...
تم عرض المسار على الخريطة مع علامات توضيحية.`
  ...
  const createHeatmap = async (intensity = 0.5, radius = 25) => {
    if (!allFeaturesData.length) {
  ...
  const findClosestIncidents = (queryLat, queryLon, queryDate = null, limit = 5) => {
    console.log(
  ...
  async function handleAction(actionObj, actionId) {
