...
testCases.forEach((testCase, index) => {
  ...
  if (routingMatch) {
    ...
    const isExplicitRouteRequest =
      /(?:أسرع|اسرع|أفضل|افضل|طريق|مسار|route|fastest|best|quickest|way|path)/i.test(
        testCase
      )
    ...
    console.log(
      `   Explicit Route Request: ${isExplicitRouteRequest ? 'Yes' : 'No'}`
    )

    if (isExplicitRouteRequest) {
      console.log(
        `   Action: Will calculate route from Dubai center to (${endLat}, ${endLon})`
      )
    } else {
